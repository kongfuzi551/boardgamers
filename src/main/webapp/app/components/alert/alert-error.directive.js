(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('boardGamesApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-boardGamesApp-error');
                var entityKey = httpResponse.headers('X-boardGamesApp-params');
                if (errorHeader) {
